...
import time
import gmsh
from VPGen.Classes import Domain
...
class PorousBox:
    ...
    def __init__(self,
        ...
        '''
        self.domain_data = domain.toJSON()
        box_a = core.Dot(coords=np.array(self.domain_data['origin'], dtype=float))
        box_b = core.Dot(coords=np.array(self.domain_data['size'], dtype=float))
        self.box = Box(box_a, box_b)
        match self.domain_data['type']:
            case 6:
                self.obj_type = ellps.Ellipsoid
            case 9:
                self.obj_type = dods.Dodecahedron
            case _:
                raise Exception('incorrect type')
        self.r_min = min(self.domain_data['radius'])
        self.r_max = max(self.domain_data['radius'])
        self.cur_num = 0
        self.target_porosity = self.domain_data['porosity'] / 100
    ...
    def gen_rand_ell(self) -> ellps.Ellipsoid:
        k_min = self.domain_data['k_min']
        k_max = self.domain_data['k_max']
        l_min = self.domain_data['l_min']
        l_max = self.domain_data['l_max']
        m_min = self.domain_data['m_min']
        m_max = self.domain_data['m_max']
